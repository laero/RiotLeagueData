...
(function () {
	...
	var Endpoints = {
		...
		staticData: {
			...
			champion: function() {
				return staticEndpoint + '/' + region + '/v1.2/champion';
			},
			item: '/v1.2/item',
	...
	RiotData.getRecentGamesBySummonerId = function(id, callback) {
		...
		options = RiotData.buildEndpoint(Endpoints.game + '/' + id + '/recent' + '?');
		requestData(options, callback);
	};
	...
	RiotData.getLeagueBySummonerId = function(id, callback) {
		...
		options = RiotData.buildEndpoint(Endpoints.league.summoner + '/' + id + '?');
		requestData(options, callback);
	};
	...
	RiotData.getLeagueEntryBySummonerId = function(id, callback) {
		...
		options = RiotData.buildEndpoint(Endpoints.league.summoner + '/' + id + '/entry' + '?');
		requestData(options, callback);
	};
	...
	RiotData.getLeagueByTeamId = function(id, callback) {
		...
		options = RiotData.buildEndpoint(Endpoints.league.team + '/' + id + '?');
		requestData(options, callback);
	};
	...
	RiotData.getLeagueEntryByTeamId = function(id, callback) {
		...
		options = RiotData.buildEndpoint(Endpoints.league.team + '/' + id + '/entry' + '?');
		requestData(options, callback);
	};
	...
	RiotData.getLeagueChallenger = function(type, callback) {
		...
		options = RiotData.buildEndpoint(Endpoints.league.challenger + '?type=' + type + '&');
		requestData(options, callback);
	};
